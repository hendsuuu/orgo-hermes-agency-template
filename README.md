...
1. A Telegram message is never delivered straight to a worker session.
...
5. Bot-authored Telegram messages are never ingested as new work.
...
5. Use the Orgo MCP only to run `seed-agent.sh --role <role>` on a new
   computer, then complete its Notion/Telegram registration.
